...
#define E_HCAR_NOT_IN_VEH "Ви повиннi бути в автомобiлi. Добавити автомобiль бiля будинку."
#define E_INVALID_HID "Невiрний будинок ID. цей будинок ID не iснує."
#define E_NO_HCAR "цей будинок ID не має автомобiля бiля будинку. Не вдаеться видалити."
...
#define I_HCAR_EXIST_ALREADY "Автомобiль бiля будинку ID %d вже є. Замiна поточного."
#define I_HCAR_CREATED "Автомобiль бiля будинку ID %d створений..."
#define I_H_DESTROYED "Будинок ID %d видалений..."
#define I_HCAR_REMOVED "Автомобiль бiля будинку ID %d Видалений..."
#define I_ALLH_DESTROYED "Всi будинки видаленi. (i того %d)"
#define I_ALLHCAR_REMOVED "Всi автомобiлi бiля будинкiв видаленi. (%d у результатi)"
...
#define E_INVALID_HCAR_MODEL "Неправильний ID моделi автомобiля. (ID моделi повиннi бути вiд 400 i до 611)"
#define I_HCAR_CHANGED "ID моделi автомобiля бiля будинку ID %d був змiнений на %d."
...
#define E_CMD_USAGE_CHANGEHCAR "Використання: /changehcar [ID будинку] [ID моделi: 400-611]"
...
#define DEBUG_OP_CMD2 "[House] %s [%d] створив автомобiль бiля будинку ID %d."
#define DEBUG_OP_CMD3 "[House] %s [%d] видалив будинок ID %d."
#define DEBUG_OP_CMD4 "[House] %s [%d] видалив автомобiль бiля будинку ID %d."
#define DEBUG_OP_CMD5 "[House] %s [%d] видалив всi будинки (%d у результатi)"
#define DEBUG_OP_CMD6 "[House] %s [%d] видалив всi автомобiлi бiля будинкiв (%d у результатi)"
...
#define DEBUG_OP_CMD12 "[House] %s [%d] змiнив ID моделi автомобiля бiля будинку ID %d на %d."
...
UnloadSystemHouse(); // Вигрузка будинкiв (также разгружает дом автомобили)
...
{
// 1. ЗАХИСТ ВiД ЗАТИСНУТИХ КЛАВiШ (Вставити сюди)
new keys, updown, leftright;
GetPlayerKeys(playerid, keys, updown, leftright);
if((keys & KEY_SPRINT) || (keys & KEY_SECONDARY_ATTACK) || (keys & KEY_FIRE)) return 1;

// 2. ВАШ СТАРИЙ КОД ЗАХИСТУ ВiД ФЛУДУ (Залишається нижче)
if(IsPlayerOnHousePickup[playerid] && LastHousePickupID[playerid] == checkpointid) return 1;

// МiКРО-ЗАХИСТ: вiдсiкає повторнi виклики стрiмера в ту саму мiлiсекунду
...
//static CPAntiFlood[MAX_PLAYERS];
//if(GetTickCount() - CPAntiFlood[playerid] < 10000) return 1;
//CPAntiFlood[playerid] = GetTickCount();
/////////////////////////////////////////////////////////////////////
...
{
// 1. ПЕРЕВiРКА КЛАВiШ (Новий код, який ми щойно додали)
new keys, updown, leftright;
GetPlayerKeys(playerid, keys, updown, leftright);
if((keys & KEY_SPRINT) || (keys & KEY_SECONDARY_ATTACK) || (keys & KEY_FIRE)) return 1;

// 2. БЛОКУВАННЯ ФЛУДУ СТРiМЕРА (Старий код, який ви скинули — його залишаємо)
...
//if(GetTickCount() - PickupAntiFlood[playerid] < 10000) return 1;
//PickupAntiFlood[playerid] = GetTickCount();
////////////////////////////////////////////////////////////////////// /
...
{
SendClientMessage(playerid, COLOUR_SYSTEM, "Сума мiстить недопустимi символи:");
SendClientMessage(playerid, COLOUR_SYSTEM, "забороненi коди, чи знак процентiв, або ~ .");
return 1;
}
new amount = floatround(strval(inputtext));
format(file, sizeof(file), FILEPATH, h);
...
format(file, sizeof(file), FILEPATH, h);
if(!dini_Exists(file)) return SendClientMessage(playerid, 0xFF0000FF, " будинка з таким ID не iснує !");
if(!strcmp(dini_Get(file, "HouseOwner"), INVALID_HOWNER_NAME, CASE_SENSETIVE)) return SendClientMessage(playerid, 0xFF0000FF, " Неможна ! Будинок вiльний !");
...
HouseLabel[h] = CreateDynamic3DTextLabel(labeltext, COLOUR_GREEN, dini_Float(file, "CPOutX"), dini_Float(file, "CPOutY"), dini_Float(file, "CPOutZ")+0.7, 8, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, dini_Int(file, "SpawnWorld"), dini_Int(file, "SpawnInterior"), -1);
#if defined GH_USE_MAPICONS
HouseMIcon[h] = CreateDynamicMapIcon(dini_Float(file, "CPOutX"), dini_Float(file, "CPOutY"), dini_Float(file, "CPOutZ"), 31, -1, dini_Int(file, "SpawnWorld"), dini_Int(file, "SpawnInterior"), -1, MICON_VD);
#endif
}
